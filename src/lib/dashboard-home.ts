...
export type DashboardHomeSummary = { monthly: HomeMonth[]; monthlyCategories: HomeMonthlyCategory[]; categories: HomeRank[]; payments: HomeRank[]; recent: HomeRecent[]; reviewCount: number; plannedCount: number; budgetTotal: number; budgetActual: number; referenceCount: number; referenceCardAmount: number };
...
export async function getDashboardHomeSummary(input: { householdId: string; from: string; to: string; monthStart: string; monthEnd: string }): Promise<DashboardHomeSummary> {
  ...
  return { monthly, monthlyCategories, categories: ranks(value.categories), payments: ranks(value.payments), recent, reviewCount: Number(value.reviewCount ?? 0), plannedCount: Number(value.plannedCount ?? 0), budgetTotal: Number(value.budgetTotal ?? 0), budgetActual: Number(value.budgetActual ?? 0), referenceCount: Number(value.referenceCount ?? 0), referenceCardAmount: Number(value.referenceCardAmount ?? 0) };
}
