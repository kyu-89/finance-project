...
export type Deposit = {
  ...
  principal: number; annualRate: number; taxRate: number; ownerMemberId: string | null;
...
export async function listDeposits(householdId: string): Promise<Deposit[]> {
  ...
  const { data, error } = await supabase.from('deposits')
    .select('id, bank_name, product_name, joined_at, maturity_date, principal, annual_rate, tax_rate, owner_member_id, withdrawal_account_id, memo, status')
  ...
  return (data ?? []).map((row) => ({
    ...
    taxRate: Number(row.tax_rate), ownerMemberId: row.owner_member_id,
...
export async function createDeposit(input: Omit<Deposit, 'id' | 'status'> & { householdId: string }): Promise<void> {
  ...
  const { error } = await supabase.from('deposits').insert({
    ...
    annual_rate: input.annualRate, tax_rate: input.taxRate, owner_member_id: input.ownerMemberId,
