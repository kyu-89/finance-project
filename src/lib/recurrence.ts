...
export function excludePausedDates(dates: string[], pauses: DateRange[]): string[] {
  return dates.filter((date) => !pauses.some((pause) => date >= pause.startDate && date <= pause.endDate));
}
