...
export type Card = {
  id: string; issuer: string; cardType: 'credit' | 'check';
  cardName: string; annualFee: number; cancellableFrom: string | null; closedAt: string | null;
  benefitSummary: string | null; paymentMethodId: string | null;
...
export async function listCards(householdId: string): Promise<Card[]> {
  ...
  const { data, error } = await supabase.from('cards')
    .select('id, issuer, card_type, card_name, annual_fee, cancellable_from, closed_at, benefit_summary, payment_method_id, memo, status')
  ...
  return (data ?? []).map((row) => ({
    id: row.id, issuer: row.issuer, cardType: row.card_type as Card['cardType'],
    cardName: row.card_name, annualFee: row.annual_fee, cancellableFrom: row.cancellable_from,
    closedAt: row.closed_at, benefitSummary: row.benefit_summary,
...
export async function createCard(input: Omit<Card, 'id' | 'status' | 'closedAt'> & { householdId: string }): Promise<void> {
  ...
  const { error } = await supabase.from('cards').insert({
    ...
    card_name: input.cardName, annual_fee: input.annualFee,
    cancellable_from: input.cancellableFrom, benefit_summary: input.benefitSummary,
    payment_method_id: input.paymentMethodId, memo: input.memo,
  });
