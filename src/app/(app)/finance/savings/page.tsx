...
export default async function FinanceSavingsPage() {
  ...
  return <div className="tds-page flex flex-col gap-6"><div><FinanceBackLink /><h1 className="tds-title mt-3">예금·적금을 관리해요</h1><p className="mt-2 text-sm text-[var(--tds-grey-700)]">납입 현황과 만기 정보를 관리해요.</p></div><SavingsProductManager deposits={deposits} savings={savings} accounts={accounts} today={todayInSeoul()} /><ProductRecurringInfo householdId={household.id} rules={rules} sourceType="saving" sourceIds={savings.map((item) => item.id)} /><ProductRecurringHistory householdId={household.id} rules={rules} sourceType="saving" sourceIds={savings.map((item) => item.id)} /></div>;
}
