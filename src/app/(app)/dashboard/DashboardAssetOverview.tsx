...
export function DashboardAssetOverview({ totalAssets, totalDebt, netWorth, debtRatio, assetRows, history, maxAsset, liquidCash, monthlyConsumption, realAssets }: { totalAssets: number; totalDebt: number; netWorth: number; debtRatio: number; assetRows: AssetRow[]; history: HistoryRow[]; maxAsset: number; liquidCash: number; monthlyConsumption: number; realAssets: Array<{ id: string; assetName: string; currentValue: number }> }) {
  ...
  return <div className="home-html-asset-view">
    <div className="home-html-kpi-grid"><HtmlKpi label="총자산" value={money(totalAssets)} meta={`금융 ${money(financial)} · 부동 ${money(nonFinancial)}`} /><HtmlKpi label="순자산" value={money(netWorth)} meta={`총자산 ${money(totalAssets)} · 부채 ${money(totalDebt)}`} accent /><HtmlKpi label="부채" value={money(totalDebt)} meta={`총자산 대비 ${(debtRatio * 100).toFixed(1)}%`} /><HtmlKpi label="비상금" value={emergencyMonths == null ? '-' : `${emergencyMonths.toFixed(1)}개월`} meta={`유동현금 ${money(liquidCash)} ÷ 월소비 ${money(monthlyConsumption)}`} /></div>
    <section className="tds-card home-html-card"><SectionTitle title="순자산 변동 추이" meta="월말 기록 기준" /><div className="home-asset-trend" role="img" aria-label="순자산 변동 추이">{history.map((item) => <div key={item.month} title={`${item.month.slice(0, 4)}년 ${Number(item.month.slice(5, 7))}월 · ${money(item.value)}`} aria-label={`${item.month} 순자산 ${money(item.value)}`}><strong>{item.month === history.at(-1)?.month ? money(item.value) : ''}</strong><span style={{ height: `${Math.max(8, item.value / maxAsset * 100)}%` }} /><small>{item.month.slice(5, 7)}월</small></div>)}</div></section>
