...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ month?: string; member?: string; preset?: string; customFrom?: string; customTo?: string }> }) {
  ...
  const monthlyByMonth = new Map(summary.monthly.map((item) => [item.month, item])); const months = Array.from({ length: 12 }, (_, index) => shiftMonth(month, index - 11)); const incomeMonthlyDetail = months.map((target) => { const rows = new Map<string, { id: string; label: string; value: number; subcategories: { id: string; label: string; value: number }[] }>(); transactions.filter((transaction) => transaction.status === 'posted' && transaction.transactionType === 'income' && reportMonth(transaction) === target).forEach((transaction) => { const id = transaction.subcategoryId ?? 'income:other'; const row = rows.get(id) ?? { id, label: incomeSubcategoryNames.get(id) ?? '기타 수입', value: 0, subcategories: [] }; row.value += transaction.amount; row.subcategories.push({ id: transaction.id, label: `${transaction.transactionDate} · ${transaction.description}`, value: transaction.amount }); rows.set(id, row); }); return { month: target, total: [...rows.values()].reduce((sum, row) => sum + row.value, 0), categories: [...rows.values()].sort((a, b) => b.value - a.value) }; }); const monthlyExpenseDetail = expenseMonthlyDetail(months); const monthlyTrend = months.map((target) => deriveMonth(monthlyByMonth.get(target) ?? emptyMonth(target))); const monthCurrent = deriveMonth(monthlyByMonth.get(month) ?? emptyMonth(month)); const current = monthCurrent; const categoryRows = summary.categories; const paymentRows = summary.payments;
  incomeSummary.monthly = incomeMonthlyDetail;
    ...
    <DashboardPrimaryTabs
      ...
      monthly={<>
        <DashboardCashflowOverview monthly={monthlyTrend} selectedMonth={month}>
          <DashboardMonthlyDetail selectedMonth={month} monthly={monthlyTrend} incomeMonthly={incomeMonthlyDetail} incomeCurrent={incomeSummary.current} expenseMonthly={monthlyExpenseDetail} expenseCurrent={categoryRows} expensePayments={paymentRows} transactionDetails={transactionDetails} />
