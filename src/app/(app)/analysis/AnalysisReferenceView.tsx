...
export function AnalysisReferenceView({ periodTransactions, paymentMethodNames, subcategoryNames }: {
  ...
  return <div className="analysis-view flex flex-col gap-4">
    ...
    <section className="tds-card tds-section-card"><SectionHeader title="결제수단별 참고 거래" description="항목을 누르면 개별 거래를 확인할 수 있어요." /><div><SimpleDrilldown rows={rows} total={total} emptyText="참고 거래가 없어요" transactionsFor={transactionsFor} extraColumn={extraColumn} /></div></section>
  </div>;
}
