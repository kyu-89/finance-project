...
// "총지출"이라는 말은 쓰지 않는다.
export function AnalysisReferenceView({ scope, months, periodTransactions, allTransactions, paymentMethodNames }: {
  ...
  return <div className="analysis-view flex flex-col gap-4">
    ...
    <section className="tds-card p-5"><h2 className="text-lg font-bold">결제수단별 참고 거래</h2><p className="mt-1 text-sm text-[var(--tds-grey-700)]">항목을 누르면 개별 거래를 확인할 수 있어요.</p><div className="mt-4"><SimpleDrilldown rows={rows} total={total} emptyText="참고 거래가 없어요" transactionsFor={transactionsFor} /></div></section>
  </div>;
}
