...
function TaskSection({ tasks }: { tasks: FinancialTask[] }) { return <section className="tds-card p-5"><SectionHeader title="재무 일정" description="기념일, 점검일과 금융 일정을 관리해요." action={<TaskDrawer />} /><div className="mt-5 divide-y divide-[var(--tds-grey-200)]">{tasks.length === 0 ? <Empty>등록된 일정이 없습니다.</Empty> : tasks.map((task) => <TaskRow key={task.id} task={task} />)}</div></section>; }
