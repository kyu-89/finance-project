...
export function WorkbookMonthlyImport({ categories, paymentMethods }: { categories: CategoryWithSubcategories[]; paymentMethods: PaymentMethod[] }) {
  ...
  const requiresPaymentMethod = validRows.some((row) => row.transactionType !== 'income');
  ...
  return <section className="tds-card p-5">
    ...
    {rows.length > 0 && <><div className="mt-4 grid gap-2 text-sm sm:grid-cols-3"><p>원본 행 <strong>{rows.length}건</strong></p><p>가져오기 대상 <strong>{validRows.length}건</strong></p><p>오류 <strong>{rows.length - parsedValidRows.length}건</strong></p></div><p className="mt-2 text-xs text-[var(--tds-grey-600)]">파일 내 중복 {duplicateRows}건은 한 번만 반영합니다. 기존 DB 중복은 서버에서 추가로 확인합니다.</p><div className="mt-3 flex flex-wrap gap-2">{sourceMonthCounts.map(([month, count]) => <span key={month} className="tds-chip">{month}: {count}건</span>)}</div><FormMessage result={state} /><form action={action}><input type="hidden" name="rows" value={JSON.stringify(validRows)} /><input type="hidden" name="sourceFileName" value={fileName} /><input type="hidden" name="totalRows" value={rows.length} /><input type="hidden" name="invalidRows" value={rows.length - parsedValidRows.length} /><input type="hidden" name="duplicateRows" value={duplicateRows} /><button disabled={pending || (requiresPaymentMethod && !paymentMethodId) || !validRows.length} className="tds-primary-button mt-4 w-full">{pending ? '가져오는 중…' : `${validRows.length}건 가져오기`}</button></form></>}
  </section>;
}
