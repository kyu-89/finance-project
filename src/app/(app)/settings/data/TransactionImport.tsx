...
export function TransactionImport({ categories, paymentMethods }: { categories: Category[]; paymentMethods: PaymentMethod[] }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    {sheets.length > 1 && <section className="tds-card p-5"><label className="flex flex-col gap-1 text-sm font-medium">가져올 시트<select className="tds-input mt-1" value={sheetName} onChange={(event) => selectSheet(event.target.value)}>{sheets.map((sheet) => <option key={sheet.name}>{sheet.name}</option>)}</select></label></section>}
    {headers.length > 0 && <>
      <section className="tds-card p-5"><h2 className="text-lg font-bold">2. 열 확인</h2><p className="mt-1 text-sm text-[var(--tds-grey-700)]">카드사마다 열 이름이 달라 자동으로 연결했어요. 맞지 않는 열만 바꿔 주세요.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{fields.map((field) => <label key={field.key} className="flex flex-col gap-1 text-sm font-medium">{field.label}{field.required && <span className="text-[var(--tds-red-500)]">필수</span>}<select className="tds-input" value={mapping[field.key] ?? ''} onChange={(event) => updateMapping(field.key, event.target.value)}><option value="">선택 안 함</option>{headers.map((header, index) => <option key={`${header}-${index}`} value={header}>{header || `(열 ${index + 1})`}</option>)}</select></label>)}</div></section>
      <section className="tds-card p-5"><h2 className="text-lg font-bold">3. 저장 방식</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="flex flex-col gap-1 text-sm font-medium">결제수단 <select className="tds-input" value={paymentMethodId} onChange={(event) => setPaymentMethodId(event.target.value)}><option value="">선택해 주세요</option>{paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select></label><label className="flex flex-col gap-1 text-sm font-medium">카테고리 열이 없거나 인식되지 않을 때 <select className="tds-input" value={fallbackCategoryId} onChange={(event) => setFallbackCategoryId(event.target.value)}><option value="">미분류로 남기기</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label></div><p className="mt-3 text-xs leading-5 text-[var(--tds-grey-500)]">카테고리를 정하지 않은 거래는 나중에 분류할 수 있도록 ‘확인 필요’로 표시해요. 같은 날짜·금액·가맹점·결제수단의 거래는 다시 올려도 중복 저장하지 않아요.</p></section>
