...
const fields: { key: ImportField; label: string; required?: boolean }[] = [
  ...
  { key: 'status', label: '취소·환불 구분' }, { key: 'category', label: '대분류' }, { key: 'subcategory', label: '소분류' }, { key: 'memo', label: '메모/할부' }, { key: 'card', label: '카드명' },
];
const won = new Intl.NumberFormat('ko-KR');

export function TransactionImport({ categories, paymentMethods }: { categories: CategoryWithSubcategories[]; paymentMethods: PaymentMethod[] }) {
  ...
  const subcategoryByName = useMemo(() => new Map(categories.flatMap((category) => category.subcategories.map((subcategory) => [subcategory.name.trim().toLocaleLowerCase(), { categoryId: category.id, subcategoryId: subcategory.id }] as const))), [categories]);
  ...
  const importableRows = useMemo(() => {
    return parsedRows.filter((row) => row.errors.length === 0 && row.transactionDate && row.amount).map((row) => {
      const categoryMatch = row.categoryName ? categoryByName.get(row.categoryName.toLocaleLowerCase()) : undefined;
      const subcategoryMatch = row.subcategoryName ? subcategoryByName.get(row.subcategoryName.toLocaleLowerCase()) : undefined;
      const inferredSubcategory = !row.subcategoryName && row.categoryName ? subcategoryByName.get(row.categoryName.toLocaleLowerCase()) : undefined;
      const categoryId = row.transactionType === 'reference' ? null : categoryMatch ?? subcategoryMatch?.categoryId ?? inferredSubcategory?.categoryId ?? fallbackCategoryId ?? null;
      const subcategoryId = row.transactionType === 'reference' ? null : subcategoryMatch?.subcategoryId ?? inferredSubcategory?.subcategoryId ?? null;
      return { transactionDate: row.transactionDate as string, transactionType: (row.transactionType === 'refund' ? 'expense' : row.transactionType) as 'income' | 'expense' | 'reference', status: (row.transactionType === 'refund' ? 'refunded' : 'posted') as 'posted' | 'refunded', amount: row.amount as number, description: row.description, categoryId, subcategoryId, paymentMethodId: row.transactionType === 'income' ? null : paymentMethodId, memo: row.memo ?? `엑셀 가져오기: ${fileName}`, needsReview: row.transactionType === 'reference' ? false : !categoryId };
    });
  }, [parsedRows, categoryByName, subcategoryByName, fallbackCategoryId, paymentMethodId, fileName]);
  const invalidCount = parsedRows.filter((row) => row.errors.length > 0).length; const referenceCount = parsedRows.filter((row) => row.errors.length === 0 && row.transactionType === 'reference').length; const categoryUnmatchedCount = parsedRows.filter((row) => row.errors.length === 0 && row.transactionType !== 'reference' && !fallbackCategoryId && !categoryByName.has(row.categoryName?.toLocaleLowerCase() ?? '') && !subcategoryByName.has(row.categoryName?.toLocaleLowerCase() ?? '') && !subcategoryByName.has(row.subcategoryName?.toLocaleLowerCase() ?? '')).length; const requiresPaymentMethod = importableRows.some((row) => row.transactionType !== 'income');
  ...
  return <div className="flex flex-col gap-5">
    <DataActionCard title="데이터 가져오기" description="카드사·은행 홈페이지에서 받은 원본 파일을 그대로 올려도 돼요. 파일은 서버에 보관하지 않고 거래로 변환해요."><label className="tds-button-secondary flex w-full cursor-pointer items-center justify-center px-4"><span>Excel·CSV 파일 선택</span><input className="sr-only" type="file" accept=".xlsx,.xls,.csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleFile(file); }} /></label>{fileName && <p className="text-sm">선택한 파일: <strong>{fileName}</strong></p>}{parseMessage && <p role="alert" className="rounded-xl bg-[oklch(0.96_0.025_22)] px-4 py-3 text-sm text-[var(--tds-red-500)]">{parseMessage}</p>}</DataActionCard>
...
function PreviewRow({ row }: { row: ParsedImportRow }) {
  ...
  const category = isReference ? null : row.categoryName || row.subcategoryName || null;
