...
export function TransactionDetailDrawer({ transaction, support, event, categories = [], paymentMethods = [], candidates = [], showEvent = false, showSupport = false, onClose }: { transaction: Transaction; support?: SupportDetail; event?: EventDetail; categories?: CategoryWithSubcategories[]; paymentMethods?: PaymentMethod[]; candidates?: DuplicateCandidate[]; showEvent?: boolean; showSupport?: boolean; onClose: () => void }) {
  ...
  return <div className="drawer-backdrop fixed inset-0 z-50" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><aside className="app-drawer ml-auto flex h-full flex-col overflow-y-auto" role="dialog" aria-modal="true" aria-label="거래 상세"><div className="app-drawer-header flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-[var(--tds-blue-600)]">거래 상세</p><h2 className="mt-1 text-xl font-bold">{transaction.description}</h2></div><Button type="button" variant="ghost" onClick={onClose} className="px-3" aria-label="상세 닫기">닫기</Button></div><div className="app-drawer-body">
    <form action={updateAction} className="monthly-drawer-form">
