...
function Summary({ label, value, suffix = '원', tone }: { label: string; value: number; suffix?: string; tone: 'income' | 'expense' | 'planned' }) { return <article className={`monthly-summary-card is-${tone}`}><span>{label}</span><strong>{value.toLocaleString('ko-KR')}{suffix}</strong></article>; }
