...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('transactions').select('transaction_date, transaction_type, amount, description, memo, status, category_id, payment_method_id').eq('household_id', householdId).is('deleted_at', null).order('transaction_date', { ascending: true });
