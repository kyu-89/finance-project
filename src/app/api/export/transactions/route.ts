...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('transactions').select('transaction_date, transaction_type, amount, description, memo, tags, status, category_id, payment_method_id').eq('household_id', householdId).is('deleted_at', null).order('transaction_date', { ascending: true });
    if (error) throw new Error(error.message);
    const headers = ['transaction_date', 'transaction_type', 'amount', 'description', 'memo', 'tags', 'status', 'category_id', 'payment_method_id'];
