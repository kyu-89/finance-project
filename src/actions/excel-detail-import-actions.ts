...
export async function importSupportEventsAction(_: ActionResult, form: FormData): Promise<ActionResult> {
  try {
    ...
    for (const row of supports) {
      ...
      await upsertSupportDetail(householdId, { transactionId: transaction.id, supportKind, eligibility: row.eligibility ? String(row.eligibility) : null, applicationPeriod: row.applicationPeriod ? String(row.applicationPeriod) : null, receivingPeriod, payoutCycle: 'one_time', expectedDate: datePattern.test(expectedDate) ? expectedDate : null, amountPerOccurrence: Number(row.amountPerOccurrence) || null, totalExpectedAmount: Number(row.totalExpectedAmount) || amount, status: String(row.status ?? 'planned'), issuer: row.issuer ? String(row.issuer) : null, contact: null, sourceUrl: null, memo: 'Excel 가져오기' });
    ...
    for (const row of events) {
      ...
      await upsertEventDetail(householdId, { transactionId: transaction.id, eventType: ['wedding', 'condolence', 'gift', 'other'].includes(String(row.eventType)) ? String(row.eventType) as 'wedding' | 'condolence' | 'gift' | 'other' : 'other', counterparty: null, relationshipGroup: null, eventDescription: description, memo: 'Excel 가져오기' });
      keys.add(key); count += 1;
    }
