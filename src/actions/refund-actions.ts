...
export async function linkRefundParentAction(_previous: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    const [{ data: refund }, { data: parent }] = await Promise.all([
      ...
      supabase.from('transactions').select('id, amount, transaction_type, flow_class, category_id, subcategory_id, payment_method_id, cost_behavior').eq('id', parentTransactionId).eq('household_id', householdId).is('deleted_at', null).single(),
    ]);
    ...
    const { error } = await supabase.from('transactions').update({ parent_transaction_id: parentTransactionId, category_id: parent.category_id, subcategory_id: parent.subcategory_id, payment_method_id: parent.payment_method_id, cost_behavior: parent.cost_behavior, needs_review: false }).eq('id', transactionId).eq('household_id', householdId).is('deleted_at', null);
