...
> 최종 갱신: 2026-08-29 · 기능 HEAD `5906d93` · main == origin/main, 워킹트리 clean
> migration 20개 로컬↔원격 동기화 · 테스트 **107개 전부 통과**(통합 테스트 실 DB 포함)
> lint ✅ / build ✅ / tsc ✅
> 운영 배포 Ready: `personal-finance-g8iupuhd4-kyu17.vercel.app` · 안정 alias `personal-finance-one-virid.vercel.app`
...
### 총평: **전체 PRD 약 62% / Phase 1(MVP) 약 82%**
...
| **Sprint 4** | **자산·금융** | 🔶 **9개 중 6개 완료 (T1~T6)** |
...
| **대출** | §10, §6.9 | 🔶 상품·상환표·RLS·UI 완료 / 조기상환 입력·반복거래 연계 남음 |
| **보험** | §11 | 🔶 상품·RLS·만기/해지 UI 완료 / 반복거래·알림 남음 |
...
| 7 | 상품 ↔ 반복거래 연계 (§5.6) | ❌ **다음 작업** |
| 8 | 기타자산·월별스냅샷·순자산 | ❌ |
| 9 | 신규 테이블 RLS 통합 테스트 | ❌ (기존 통합 포함 전체 107개는 통과) |

**순서 제약:** T3→T4, T5→T6, T7은 T4+T6 이후, T9는 마지막.

Task 7에서 적금 500,000원은 `saving`, 대출원금 900,000원은 `debt_principal`, 대출이자는 `finance_cost`로 반드시 분리한다. 상품 해지·만기·상환완료 시 향후 planned 생성도 즉시 중단해야 한다.
...
```
1. Sprint 4 Task 7 (상품 ↔ 반복거래) 부터 순서대로
   → docs/superpowers/plans/2026-08-29-sprint4-assets-finance.md 에 코드 수준으로 있음
   → saving / debt_principal / finance_cost 흐름을 섞지 말 것
   → 상품 상태가 active를 벗어나면 반복규칙 ended + 미래 planned 중단
