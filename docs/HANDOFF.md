...
> 최종 갱신: 2026-08-29 · 기능 HEAD `1fc6ddd` · main == origin/main, 워킹트리 clean
> migration 23개 로컬↔원격 동기화 · 테스트 **129개**(unit 87 + integration 42, 실 DB 포함) 통과
> lint ✅ / build ✅ / tsc ✅
> 운영 배포 Ready: `personal-finance-2k0bzt3y8-kyu17.vercel.app` · 안정 alias `personal-finance-one-virid.vercel.app`
...
### 총평: **전체 PRD 약 78% / Phase 1(MVP) 약 99%**
...
| **대시보드 (KPI·추이·드릴다운)** | §8, §20.1 | 🔶 8종 기간 필터·9 KPI·이전/전년 비교·필수 차트·카테고리/결제수단·구성원 필터 및 `구성원→대분류→거래` 드릴다운 완료 / 과거 구성원별 자산 스냅샷 남음 |
...
### 이번에 완료한 대시보드 (`55261fe`, `889fe25`, `1fc6ddd`)
...
- PRD 필수 시각화인 예산 대 실제·자산 구성·월별 YoY 비교 추가, 현금흐름 차트에 잔여현금 반영
- YoY 12개월 차트를 위해 조회 범위를 24개월로 확장하되 기존 페이지네이션 유지
- 이번 달·지난 달·YTD·작년·최근 3/6/12개월·사용자 지정 전역 기간 필터
- 동일 길이 이전 기간과 전년 동기 계산을 KPI·예산·차트·드릴다운에 일관 적용(윤년 포함 테스트)

다음 구현 순서: Excel Import.

### 구성원 필터 후속 구현

- 대시보드에 가구 전체·구성원·미지정 전역 필터를 추가했다.
- 거래 귀속은 비용대상(`beneficiary_member_id`) 우선, 없으면 거래자(`payer_member_id`)를 사용해 한 거래가 한 구성원에만 집계된다.
- 선택 구성원은 KPI·현금흐름·카테고리·결제수단·YoY·원인 거래에 일관 적용되고, 구성원별 소비에서 대분류·거래로 내려간다.
- 현재 계좌·예적금·대출·기타자산은 상품 명의자로 필터링한다. 저장된 월별 스냅샷은 구성원 차원이 없어 과거 자산 비교·추이는 가구 전체로 표시한다.
