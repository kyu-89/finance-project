...
This get-or-create is now self-healing on every call: a fresh call creates both rows; a
sequential repeat call finds the existing household via `selectHousehold` and no-ops through
`ensureSelfMember`'s own existence check; a household-insert race is caught by unique-violation
code `23505` and resolved by re-reading the winner's row instead of throwing; and a prior call
that created the household but failed before creating its member row is repaired on the very
next call (rather than being permanently stuck, since `existing` would otherwise short-circuit
before ever reaching the member insert). The one residual gap — `household_members` has no DB
constraint to make its own insert race fully atomic — is called out in the ledger as a follow-up
migration to consider, not fixed here (no unit test either, for the same reason: this is exactly
what an integration test against the real linked database would need to exercise, and no such
test exists in this plan yet).
