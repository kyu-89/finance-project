...
with filtered as (
  ...
  select to_char(date_trunc('month', t.transaction_date), 'YYYY-MM') as month,
    t.effective_category_id, coalesce(c.name, '미분류') as label, sum(t.signed_amount) as value
  ...
  group by 1, t.effective_category_id, c.name
), monthly_categories as (
  select month, sum(value) as total,
    jsonb_agg(jsonb_build_object('id', coalesce(effective_category_id::text, 'unassigned'), 'label', label, 'value', value) order by value desc) as categories
...
select jsonb_build_object(
  ...
  'monthlyCategories', coalesce((select jsonb_agg(jsonb_build_object('month', mc.month, 'total', mc.total, 'categories', mc.categories) order by mc.month) from monthly_categories mc), '[]'::jsonb),
  ...
  'reviewCount', (select count(*) from normalized where transaction_date between p_month_start and p_month_end and needs_review),
  'plannedCount', (select count(*) from normalized where transaction_date between p_month_start and p_month_end and status = 'planned'),
  'budgetTotal', (select total from budget_summary), 'budgetActual', (select actual from budget_summary)
);
$$;
