...
with filtered as (
  ...
  select id, transaction_date, transaction_type, flow_class, amount, description
  from filtered
  where transaction_date between p_month_start and p_month_end and status = 'posted'
  order by transaction_date desc, created_at desc limit 5
), budget_summary as (
  select
    coalesce(sum(b.amount) filter (where b.transaction_type = 'expense'), 0) as total,
    coalesce((select sum(t.amount) from filtered t
      where t.transaction_date between p_month_start and p_month_end
        and t.status = 'posted' and t.flow_class = 'consumption' and t.include_in_budget), 0) as actual
  from public.budgets b
  where b.household_id = p_household_id
    and b.year = extract(year from p_month_start)::int
    and b.month = extract(month from p_month_start)::int
)
select jsonb_build_object(
  'monthly', coalesce((select jsonb_agg(to_jsonb(m) order by m.month) from monthly m), '[]'::jsonb),
  'categories', coalesce((select jsonb_agg(to_jsonb(c)) from category_rows c), '[]'::jsonb),
  'payments', coalesce((select jsonb_agg(to_jsonb(p)) from payment_rows p), '[]'::jsonb),
  'recent', coalesce((select jsonb_agg(to_jsonb(r)) from recent_rows r), '[]'::jsonb),
  'reviewCount', (select count(*) from filtered where transaction_date between p_month_start and p_month_end and needs_review),
  'plannedCount', (select count(*) from filtered where transaction_date between p_month_start and p_month_end and status = 'planned'),
